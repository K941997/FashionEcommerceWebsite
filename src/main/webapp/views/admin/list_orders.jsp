		   	...
		   	<div class="card">
		         <div class="card-body">
	        			...
	        			<c:set var="page" value="${requestScope.page }"></c:set>
					  	<ul class="pagination d-flex justify-content-center ">
					    	<c:forEach begin="${1}" end="${requestScope.num}" var="i">
					    		<li class="page-item ">
		           					<a  class="page-link ${i==page?"active":""}"  href="list-order-admin?page=${i}">${i}</a>
		           				</li>
		           			</c:forEach>
					  </ul>
					</nav>
		           ...
		           <table class="table">
		             <thead>
		               <tr>
		               	<th hidden="" scope="col">Real ID</th>
		             ...
		             <tbody>
		             	<c:forEach items="${requestScope.listOrders}" var="order">
		             		<tr>
		             			<td hidden="" scope="row">${order.orderId}</td>
								<td scope="row">${order.customId}</td>
								<td scope="row">${order.userFullName}</td>
								<td scope="row">${order.userPhone}</td>
								<td scope="row">${order.addressShipment}</td>
								<td scope="row"><fmt:formatNumber value="${order.totalmoney}" type="currency" currencyCode="VND" /></td>
								<td scope="row">
								...
								<td scope="row">
									...
									<c:if test="${order.status == 2}">
										<a class="btn btn-sm btn-danger" href="cancel-order-admin?orderId=${order.orderId}">Cancel</a>
										<a class="btn btn-sm btn-success" href="complete-order-admin?orderId=${order.orderId}">Complete</a>
									</c:if>
								...
								<td class="column-5"><a class="btn btn-sm btn-info" style="color: white;" href="detail-order-admin?orderId=${order.orderId}">View Detail</a></th>
		                	</tr>
		             	</c:forEach>
		             </tbody>
		           </table>
