...
//		//Search:
		...
		productsSearch =  productDAO.searchProductsByKey(keySearch);
		
//		//Paginate (Phân trang) - Theo Danh Sách Products Search:
		int page, numberPage = 6;
		int size = productsSearch.size();
		...
		List<Product> productsSearchPagination = new ArrayList<Product>();
		productsSearchPagination = productDAO.getProductsByPage(productsSearch, start, end);
		req.setAttribute("page", page);
		req.setAttribute("num", num);
		req.setAttribute("productsSearchPagination", productsSearchPagination);
...
//		//FilterCheckbox Prices (Lọc theo Nhiều Prices): Tách riêng JSP/Servlet:
		...
		//FilterCheckbox Prices (Lọc theo Nhiều Prices):
		...
		//Final
		req.getRequestDispatcher("/views/admin/search_product.jsp").forward(req, resp);
	}
